...
import { checkShortListed, deleteShortListedCandidate, fetchShortListed, shortListedCandidate } from '@/backend/employerBackend';
...
const CandidateDetail = (props: any) => {
    ...
    const [shorted, setShorted] = useState<any>()
    ...
    const handleShortList = async (id: string) => {
        ...
        shortListedCandidate(props.jobId, id).then((res) => {
            ...
            res !== null && fetchShortListed(props.jobId).then((res: any) => {
                if (res.documents) {
                    setListed(res.documents)
                    setLoading(false)
                    toast.success('Added To ShortList');
                    checkShortList(props.detailData.Id)
                } else {
    ...
    const checkShortList = (id: string) => {
        setShortLoading(true)
        checkShortListed(props.jobId, id).then((res: any) => {
            res && res.documents && res.documents[0] && setFileId(res?.documents[0].$id)
            if (res.total == 0) {
                setShorted(false)
    ...
    useEffect(() => {
        props.detailData && checkShortList(props.detailData.Id)
    }, [props.detailData, listed])
    const deleteShortListed = async () => {
        setLoading(true)
        deleteShortListedCandidate(fileId).then((res) => {
            fetchShortListed(props.jobId).then((res) => {
                setLoading(false)
                toast.success('Removed From ShortList');
                res && res.documents && props.setShortListed(res.documents);
        ...
        <div className="flex flex-col gap-y-3 rounded-xl border-2 w-full md:flex-grow">
            <div className='w-full flex flex-col gap-5 p-5 border-b-2 '>
                ...
                <div className='w-full flex'>
                    ...
                    {((!loading && props.short) || (!props.short && shorted == false)) && <div className='bg-black rounded-lg'> <button type='button' disabled={loading ? true : false} onClick={props.short == true ? () => deleteShortListed() : () => handleShortList(props.detailData.Id)} className='bg-gradient-to-r from-[#00A82D] to-[#0CBC8B] text-textW px-10 py-2 cursor-pointer rounded-lg buttonBounce'>
                        ...
                        {!shortLoading && !loading && !props.short && shorted == false && <span>Add to Shortlist</span>}
                    </button>
                    </div>}

                    {props.short == false && shorted == true && <div className='bg-gray-100 text-gray-500 px-10 py-2 cursor-pointer rounded-lg'>
