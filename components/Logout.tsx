...
const Logout = (props: any) => {
    ...
    return (
        <ConfirmModal isOpen={props.openLogout} handleClose={() => props.setOpenLogout(!props.openLogout)}>
            <div className="mx-2 pb-10 w-full pl-5 bg-textW rounded-2xl flex flex-col gap-y-5 items-center justify-center pt-10 md:pl-8 pr-5 md:w-2/3 lg:w-1/2 md:mx-0">
                ...
                <div className="flex gap-10 flex-wrap w-full justify-center">
                    <button
                        onClick={() => props.setOpenLogout(!props.openLogout)}
                        type="button"
                        className="bg-gray-200 h-14 w-full md:w-60 rounded-xl order-1 sm:order-2"
                    >
                        No
                    </button>
                    <button onClick={handleLogout} disabled={loading ? true : false} type="button" className="bg-black w-full md:w-60 rounded-xl text-textW h-14 order-1 sm:order-2">
